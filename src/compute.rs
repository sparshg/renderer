...
use crate::{
    renderer::{Attach, BindGroupBuilder, Pipeline, PipelineBuilder, SurfaceContext},
    VERTEX_STRUCT_SIZE,
};

pub const POS: &[[f32; 3]; 77] = &[
    // [1., 0., 1.],
    // [1., 0.41421357, 0.],
...
impl ComputePipeline {
    pub fn new(ctx: &SurfaceContext) -> Self {
        ...
        let bind_group = bind_group_layout.attach(
            ctx,
            "Compute Bind Group",
            vec![
                point_buff.as_entire_binding(),
                vert_buff.as_entire_binding(),
                ind_buff.as_entire_binding(),
            ],
        );
